...
function cleanAllDuplicates() {
  ...
  // 找到所有使用重复图片的游戏（313KB的重复图片）
  const duplicateImagePattern = /-unique-175393582618\d+\.(jpg|webp)$/;
  const gamesToRemove = games.filter(game => {
    const imagePath = game.image;
    return duplicateImagePattern.test(imagePath);
  });
